...
def main():
    ...
    data = np.genfromtxt(r'diffusion_maps\\artificial_class_data.csv', delimiter=',', skip_header=1)
    ...
    ax1.scatter(X[:, 0], X[:, 1], X[:, 2], c=color, cmap=cm.plasma)
    ax1.set_title('Dados Originais')

    # Plot dos dados transformados em 2D (Configuração 1)
    ax2 = fig.add_subplot(132)
    ax2.scatter(Ut[:, 1], Ut[:, 2], c=color, cmap=cm.plasma)
    ...
    ax3.scatter(Ut_new[:, 1], Ut_new[:, 2], c=color, cmap=cm.plasma)
